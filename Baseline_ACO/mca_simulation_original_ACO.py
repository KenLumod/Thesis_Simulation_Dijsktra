...
class MCASimulation:
    ...
    def compute_flow_directions(self):
        ...
        # Parameters: n_ants=200, iterations=2000 to ensure coverage
        # Capture History for Animation
        self.directions, self.pheromones, self.aco_history = run_aco_pathfinding(
            self.graph, valid_exits, self.road_cells, max_iterations=2000, n_ants=200, record_history=True
        )
        
        # Pre-calc flow vectors (Standard Visualization)
        self.flow_vectors = {}  # Reset
    ...
    def export_to_excel(self, filename="simulation_results_aco_1.xlsx"):
    ...
    def run(self, steps=100):
        ...
        self.export_to_excel(filename=os.path.join(script_dir, "simulation_results_aco_1.xlsx"))
    ...
    def animate_results(self):
        ...
        def update_selection_text(frame_idx):
            ...
            # PHASE 1 Check
            if hasattr(self, 'aco_history') and frame_idx < len(self.aco_history):
                 # ACO Phase Selection Info
                 step_data = self.aco_history[frame_idx]
                 ph = step_data['pheromones']
                 
                 # Approx pheromone level on this cell?
                 # Sum of outgoing/incoming?
                 # Let's show max incoming
                 max_p = 0.0
                 count_p = 0
                 for (u, v), strength in ph.items():
                     if u == cid or v == cid:
                         max_p = max(max_p, strength)
                         count_p += 1
                 
                 selected_cell_text.set_text(
                    f"Cell ID: {cid}\n"
                    f"ACO Phase\n"
                    f"Pheromone: {max_p:.2f}\n"
                    f"Edges: {count_p}"
                 )
                 return
            ...
            sim_frame = frame_idx
            if hasattr(self, 'aco_history'):
                 sim_frame = frame_idx - len(self.aco_history)
        ...
        def update(frame):
            ...
            # --- PHASE 1: ACO VISUALIZATION ---
            if hasattr(self, 'aco_history') and frame < len(self.aco_history):
                # We are in ACO Phase
                step_data = self.aco_history[frame]
                ant_positions = step_data['ants']
                pheromones = step_data['pheromones']
                
                # 1. Pheromone Heatmap
                # Calculate aggregated pheromone per cell (sum of incoming edges?)
                # Actually, simpler: show MAX incoming pheromone for each cell
                p_values = {}
                for (u, v), strength in pheromones.items():
                    # u->v is the edge ants walked.
                    # Pheromone is on this edge.
                    # Visualize on 'u' or 'v'?
                    # Let's map it to 'u' (source of edge) to show trail
                    curr = p_values.get(u, 0.0)
                    p_values[u] = max(curr, strength)
                
                ratios = []
                for idx in self.road_cells['id']:
                     val = p_values.get(idx, 0.0)
                     # Normalize for viz (e.g. max expected ~ 100)
                     norm = min(val / 50.0, 1.0) 
                     ratios.append(norm)
                
                collection.set_array(np.array(ratios))
                collection.set_cmap('viridis') # different cmap for ACO
                ...
                if ant_positions:
                     # Find geometry
                     ant_geom = self.road_cells[self.road_cells['id'].isin(ant_positions)]
                     if not ant_geom.empty:
                          centroids = ant_geom.geometry.centroid
                          # Plot ants as small dots
                          centroids.plot(ax=ax, color='white', marker='.', markersize=20, zorder=8, alpha=0.7)
                          self.remaining_artist = ax.collections[-1]

                info_text.set_text(
                    f"🐜 ACO PHASE\n"
                    f"Step: {frame * 10}\n" # x10 because we subsampled
                    f"Ants Active: {len(ant_positions)}\n"
                    f"Pheromones: {len(pheromones)} edges"
                )
                
                ax.set_title("Phase 1: Ant Colony Optimization (Pheromone Trail Building)", fontsize=14, fontweight='bold')
            ...
            sim_frame = frame
            if hasattr(self, 'aco_history'):
                 sim_frame = frame - len(self.aco_history)
            
            # Safety check
            if sim_frame < 0 or sim_frame >= len(self.history):
                return collection, info_text
            ...
            elif self.view_mode == 'pheromones':
                 ax.set_title("Phase 2: Evacuation (Overlay: ACO Trails)", fontsize=14, fontweight='bold')
            else:
            ...
            if self.view_mode == 'occupancy':
                ...
                for idx in self.road_cells['id']:
                     ...
                     ratios.append(r)
                collection.set_array(np.array(ratios))
                # collection.set_cmap('turbo') # Done above
                collection.set_clim(0, 1.0)
                ...
                # Pheromone Overlay (Static Final State if in Phase 2)
                if self.quiver: 
                    self.quiver.remove()
                    self.quiver = None
                if self.remaining_artist:
                ...
                p_values = {}
                # Use self.pheromones (Final State)
                for (u, v), strength in self.pheromones.items():
                    curr = p_values.get(u, 0.0)
                    p_values[u] = max(curr, strength)
        ...
        ax_btn = plt.axes([0.50, 0.04, 0.12, 0.04]) # Shifted left
        self.btn_casualty = Button(ax_btn, 'Casualties', color='salmon', hovercolor='red')
        
        # 3rd Button: ACO Path
        ax_btn_aco = plt.axes([0.65, 0.04, 0.12, 0.04])
        self.btn_aco = Button(ax_btn_aco, 'Pheromones', color='lightgreen', hovercolor='lime')
        ...
        def set_view_mode(mode):
            ...
            self.btn_aco.color = 'lightgreen'
            
            if mode == 'casualties':
                self.btn_casualty.color = 'red' # Active
            ...
            elif mode == 'pheromones':
                self.btn_aco.color = 'lime' # Active
                ax.set_title("ACO Pheromone Trails (Final State)", fontsize=14, fontweight='bold')
        ...
        def toggle_aco(event):
            if self.view_mode == 'pheromones':
                set_view_mode('occupancy')
            else:
                set_view_mode('pheromones')
            
        self.btn_casualty.on_clicked(toggle_casualty)
        self.btn_aco.on_clicked(toggle_aco)
        ...
        # Total Frames = ACO Frames + Sim Frames
        total_frames = len(self.history)
        if hasattr(self, 'aco_history'):
             total_frames += len(self.aco_history)
